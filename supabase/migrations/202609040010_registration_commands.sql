create function public.registration_command(p_trip_id uuid,p_command text,p_request_id uuid,
...
begin
 ...
 if p_command in ('register','accept_offer','update_response') then
   ...
   if p_command<>'accept_offer' then
     ...
     if p_data->'waiverAgreed'='true'::jsonb then
       ...
       insert into public.registration_signatures(trip_id,user_id,original_signer_id,waiver_id,signature_name)
         values(p_trip_id,target,target,s.waiver_id,trim(p_data->>'signatureName')) on conflict(user_id,waiver_id) do nothing;
     end if;
   end if;
 ...
 when 'guardian_review' then
   ...
   insert into public.registration_guardian_reviews(trip_id,user_id,waiver_id,reviewer_id,evidence)
   values(p_trip_id,target,case when s.waiver_required then s.waiver_id end,uid,trim(p_data->>'evidence'))
   on conflict(trip_id,user_id) do update set waiver_id=excluded.waiver_id,reviewer_id=uid,evidence=excluded.evidence,reviewed_at=now();
