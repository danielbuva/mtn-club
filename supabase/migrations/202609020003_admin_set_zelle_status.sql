-- Let payment reviewers manage an application's Zelle review state even when
-- the applicant has not submitted a claim. Applicant-originated claims remain
-- distinguishable through claim_source.

-- Repair the initial phase-one migration: membership activation must not write
-- a mailing-consent event, and consent changes must append their own event.
