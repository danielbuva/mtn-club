...
## Live theme and focused registration

The default dark theme uses the same warm editorial tokens for page surfaces, controls, actions, and sidebars. The preserved red variant requires both `dark` and `theme-red-dark` on the root and is not exposed in theme controls. The participant registration page starts with the actual trip name and progress. It omits the old page navigation, management link, summary panel, and recent-update history. Eligibility and guardian actions appear at review, and command errors appear inside the form. A top-right X opens an exit overlay with draft saving, registration cancellation, and a non-destructive close action; no cancellation button appears below the form. Draft saving uses the same normalization function and never signs waivers. Organizer roster and operations pages keep their existing navigation.
